...
class AccountService
{
    ...
    public function storeAccount(array $data): Account
    {
        ...
        if ($data['type'] === 'MINEUR') {
            ...
            if (!$guardian || Carbon::parse($guardian->date_naissance)->age < 18) {
    ...
    public function addCoHolder(int $accountId, int $userId): void
    {
        ...
        if ($account->users()->where('user_id', $userId)->exists()) {
            throw new Exception("User is already a holder of this account.");
        }
    ...
    public function convertToCurrentAccount(int $accountId): Account
    {
        ...
        $pivot = $account->users()->where('user_id', auth()->id())->first();
        if (!$pivot || $pivot->pivot->relation_type !== 'guardian') {
    ...
    public function authorizeHolder(Account $account): void
    {
    ...
    public function isGuardianOf(Account $account, int $userId): bool
    {
        $user = $account->users()->where('user_id', $userId)->first();
        return $user && $user->pivot->relation_type === 'guardian';
    }

    private function requireActive(Account $account): void
    {
        if ($account->status !== 'ACTIVE') {
            throw new Exception("This operation requires an active account.");
        }
    }
}